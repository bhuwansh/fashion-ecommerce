...
if($isset($_POST['submit'])){
    ...
    $res=mysqli_query($con,"select * from categories where id='$categories'");
    $row=mysqli_fetch_assoc($res);
    if($check>0){
        if($isset($_GET['id']) && ($isset($_GET['id'])!='')){
    ...
    if($msg==''){
        if($isset($_GET['id']) && ($isset($_GET['id'])!='')){
            ...
            mysqli_query($con,"insert into categories(categories,values)values('$categories where id='$id'");
        }
        header('location:categories.php');
        die();
    }
